...
transformIndirection(ParseState *pstate, A_Indirection *indir)
{
	...
	foreach(li, indir->indirection)
	{
		...
		else
		{
			...
			sbsref->refcontainertype = arrtype;
			sbsref->refelemtype = elemtype;
			sbsref->reftypmod = arrtypmod;
			sbsref->refupperindexpr = list_make1(uidx);
			sbsref->reflowerindexpr = (ind->is_slice ? list_make1(lidx) : NIL);
			sbsref->refexpr = (Expr *) res;

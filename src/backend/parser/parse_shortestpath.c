...
makeVertexScan(CypherPath *cpath)
{
	...
	col = makeResTarget((Node *) aa, CTE_COLNAME_VARR);
	...
	sel->fromClause = list_make2(rf, v);
	...
	vscan = makeNode(RangeSubselect);
	vscan->subquery = (Node *) sel;
	vscan->alias = makeAliasNoDup(CTE_VNAME, NIL);
	vscan->lateral = true;

 ...
 * indeed saves later search effort in _ei_pagedel.  The caller should
	...
	if (poffset >= maxoff)
	{
		...
		if (poffset == P_FIRSTDATAKEY(opaque))
		{
			...
			*rightsib = opaque->btpo_next;
			...
			/*
			 * Like in _ei_pagedel, check that the left sibling is not marked
			 ...
			 * _ei_mark_page_halfdead performed on the leaf level.
			 */
...
/*
 * _ei_pagedel() -- Delete a page from the b-tree, if legal to do so.
 *
...
_ei_mark_page_halfdead(Relation rel, Buffer leafbuf, BTStack stack)
{
	 ...
	 * _ei_lock_branch_parent should have guaranteed it has the expected

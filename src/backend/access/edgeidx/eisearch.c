...
/*
 *	_ei_search() -- Search the tree for a particular scankey,
		   ...
		   Buffer *bufP, int access, Snapshot snapshot)
{
	...
	*bufP = _bt_getroot(rel, access);
	...
	for (;;)
	{
		 ...
		 *
		 * In write-mode, allow _ei_moveright to finish any incomplete splits
...
/*
 *	_ei_moveright() -- move right in the btree if necessary.
 *
...
/*
 *	_ei_binsrch() -- Do a binary search for a key on a particular page.
 *
 * The passed scankey must be an insertion-type scankey (see nbtree/README),
 * but it can omit the rightmost column(s) of the index.
 *
 ...
 * item strictly greater than scankey.
 *
 * On a leaf page, _ei_binsrch() returns the OffsetNumber of the first
 * key >= given scankey, or > scankey if nextkey is true.  (NOTE: in
 * particular, this means it is possible to return a value 1 greater than the
 * number of keys on the page, if the scankey is > all keys on the page.)
 *
 * On an internal (non-leaf) page, _ei_binsrch() returns the OffsetNumber
 * of the last key < given scankey, or last key <= given scankey if nextkey
 * is true.  (Since _ei_compare treats the first data key of such a page as
 ...
 * the given page.  _ei_binsrch() has no lock or refcount side effects
 * on the buffer.
 */
OffsetNumber
			...
			bool nextkey)
{
	 ...
	 * There must be one if _ei_compare() is playing by the rules.
	 */
...
/*----------
 *	_ei_compare() -- Compare scankey to a particular tuple on the page.
 *
 * The passed scankey must be an insertion-type scankey (see nbtree/README),
 * but it can omit the rightmost column(s) of the index.
 *
			...
			OffsetNumber offnum)
{
	...
	for (i = 1; i <= keysz; i++)
	{
		...
		else
		{
			 ...
			 * _ei_compare as comparing the scankey to the index item, we have
...
/*
 *	_ei_first() -- Find the first item in a scan.
 *
...
_ei_first(IndexScanDesc scan, ScanDirection dir)
{
	 ...
	 * code below.
	 *
	 * If nextkey = false, _ei_search and _ei_binsrch will locate the first
 ...
 *
 * This is used by _ei_first() to set up a scan when we've determined
 * that the scan must start at the beginning or end of the index (for
 * a forward or backward scan respectively).  Exit conditions are the
 * same as for _ei_first().
 */
static bool
_bt_endpoint(IndexScanDesc scan, ScanDirection dir)
{
	 ...
	 * version of _ei_search().  We don't maintain a stack since we know we
	 * won't need it.
	 */

...
CreateDirAndVersionFile(char *dbpath, Oid dbid, Oid tsid, bool isRedo)
{
	...
	snprintf(agversionfile, sizeof(agversionfile), "%s/%s", dbpath, "AG_VERSION");
	...
	fd = OpenTransientFile(agversionfile, O_WRONLY | O_CREAT | O_EXCL | PG_BINARY);
	if (fd < 0 && errno == EEXIST && isRedo)
		fd = OpenTransientFile(agversionfile, O_WRONLY | O_TRUNC | PG_BINARY);

	if (fd < 0)
		ereport(ERROR,
				(errcode_for_file_access(),
				 errmsg("could not create file \"%s\": %m", agversionfile)));
	...
	if ((int) write(fd, buf, nbytes) != nbytes)
	{
		...
		ereport(ERROR,
				(errcode_for_file_access(),
				 errmsg("could not write to file \"%s\": %m", agversionfile)));
	}
	pgstat_report_wait_end();

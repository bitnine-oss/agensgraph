...
static void StoreCatalogInheritance(Oid classId, Oid relationId, List *supers);
static void StoreCatalogInheritance1(Oid relationId, Oid parentOid,
						 int16 seqNumber, Relation inhRelation);
static void StoreLabelInheritance1(Oid relationId, Oid parentOid,
						 int16 seqNumber, Relation inhRelation);
			   ...
			   ObjectAddress *typaddress)
{
	...
	/*
	 * Check duplication in graph label
	 */
	if (IsA(stmt, CreateVLabelStmt) ||
		IsA(stmt, CreateELabelStmt))
	{
		...
		{
			tup = SearchSysCache1(LABELOID, ObjectIdGetDatum(relOid));
			if (HeapTupleIsValid(tup))
				ereport(ERROR,
						(errcode(ERRCODE_DUPLICATE_TABLE),
						 errmsg("Graph label \"%s\" already exists", relname)));
		}
	}
	...
	{
		InsertAgLabelTuple(relationId, relname, LABEL_KIND_VERTEX, inheritOids);
		StoreCatalogInheritance(InheritsLabelId, relationId, inheritOids);
	}
	else if (nodeTag(stmt) == T_CreateELabelStmt)
	{
		InsertAgLabelTuple(relationId, relname, LABEL_KIND_EDGE, inheritOids);
		StoreCatalogInheritance(InheritsLabelId, relationId, inheritOids);
	}
...
RemoveRelations(DropStmt *drop)
{
	...
	foreach(cell, drop->objects)
	{
			...
			drop->removeType == OBJECT_ELABEL)
		{
			...
			else if (strcmp(rel->schemaname, AG_GRAPH) != 0)
			{
				ereport(ERROR,
			...
			if (HeapTupleIsValid(tup))
				ereport(ERROR,
						(errcode(ERRCODE_DEPENDENT_OBJECTS_STILL_EXIST),
						 errmsg("cannot drop %s because ag label depends on it",
							 rel->relname),
						 errhint("Use DROP VLABEL/ELABEL to drop the graph label.")));
...
RemoveLabels(DropStmt *drop)
{
	...
	foreach(cell, drop->objects)
	{
		...
		children = find_inheritance_children(InheritsLabelId, relOid, NoLock);
		...
		/* delete ag_inherit */
		RelationRemoveInheritance(InheritsLabelId, relOid);
	}
...
StoreCatalogInheritance(Oid classId, Oid relationId, List *supers)
{
	...
	foreach(entry, supers)
	{
		...
		if (classId == InheritsRelationId)
			...
			StoreLabelInheritance1(relationId, parentOid, seqNumber, relation);
		seqNumber++;
	}
...
static void
StoreLabelInheritance1(Oid relationId, Oid parentOid,
					   int16 seqNumber, Relation inhRelation)
{
	TupleDesc	desc = RelationGetDescr(inhRelation);

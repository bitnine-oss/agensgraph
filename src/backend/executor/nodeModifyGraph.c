			 ...
			 TupleTableSlot *slot, bool inPath)
{
	...
	snprintf(sqlcmd, SQLCMD_BUFLEN, SQLCMD_CREAT_VERTEX, graph, label);
		   ...
		   Graphid end, TupleTableSlot *slot, bool inPath)
{
	...
	Oid			argTypes[SQLCMD_EDGE_NPARAMS] = {GRAPHIDOID, GRAPHIDOID,
												 JSONBOID};
	int			ret;
	char	   *graph = get_graph_path();
	Datum		edge = (Datum) NULL;

	snprintf(sqlcmd, SQLCMD_BUFLEN, SQLCMD_CREAT_EDGE, graph, gedge->label);
...
vertexHasEdge(Datum vid)
{
	...
	snprintf(sqlcmd, SQLCMD_BUFLEN, SQLCMD_DETACH, graph);

	values[0] = vid;
	argTypes[0] = GRAPHIDOID;

	ret = SPI_execute_with_args(sqlcmd, SQLCMD_DETACH_NPARAMS, argTypes,
								values, NULL, false, 1);
...
deleteVertexEdges(Datum vid)
{
	...
	snprintf(sqlcmd, SQLCMD_BUFLEN, SQLCMD_DEL_EDGES, graph);
...
deleteElem(char *relname, int64 id)
{
	...
	snprintf(sqlcmd, SQLCMD_BUFLEN, SQLCMD_DEL_ELEM, graph, relname);

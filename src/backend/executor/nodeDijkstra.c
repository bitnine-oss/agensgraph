...
proj_path(DijkstraState *node)
{
	...
	vertex = end = (vnode *) hash_search(
			node->visited_nodes, &node->target_id, HASH_FIND, &found);
	Assert(found);
...
ExecDijkstra(DijkstraState *node)
{
	...
	while (!pairingheap_is_empty(node->pq))
	{
		...
		frontier = (vnode *) hash_search(
				node->visited_nodes, &min_pq_entry->to, HASH_FIND, &found);
		Assert(found);
		...
		for (;;)
		{
			...
			neighbor = (vnode *) hash_search(
					node->visited_nodes, &to_val, HASH_ENTER, &found);
...
ExecInitDijkstra(Dijkstra *node, EState *estate, int eflags)
{
	...
	ExecSetSlotDescriptor(dstate->selfTupleSlot,

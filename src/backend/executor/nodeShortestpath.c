...
Datum
ExecShortestpathProjectEvalArray(Oid element_typeid, unsigned char *elems, long len, ExprContext *econtext)
{
	MemoryContext  oldContext;
	Datum         *values;
	bool          *nulls;
	...
	{
		values = palloc0(len * sizeof(*values));
		nulls = palloc0(len * sizeof(*nulls));
		for (i = 0; i < len; i++)
		{
			if (elembyval == true)
			{

...
static char *deparse_prop_expression_pretty(Node *expr, List *dpcontext,
						  bool forceprefix, bool showimplicit,
						  int prettyFlags, int startIndent);
static char *removePropertiesOpExpr(const char *src);
...
/*
 * Prints elements of the properties(right operand of '#>>') only.
 * Other expressions are printed originally.
 */
static char *removePropertiesOpExpr(const char *src)
{
	...
	int	len_prefix  = strlen(prop_prefix);
	int	len_postfix = strlen(prop_postfix);
	int	len_delim   = strlen(prop_delim);

	initStringInfo(&strInfo);

	prevEnd = (char *) src;
	while ((curr = strstr(prevEnd, prop_prefix)) && (curr != NULL))
	{
		char   *begin;
		char   *end;
		char   *sep = "";
		...
		appendBinaryStringInfo(&strInfo, prevEnd, curr - prevEnd);

		/* find a properties expression */
		begin = curr + len_prefix;
		end = strstr(begin, prop_postfix);
		if (!end)
		...
		while (curr < end)
		{
			if (*curr == '\'')	/* nested field */
			{
				char *startPos = curr + 1;
				char *endPos = strchr(startPos, '\'');

				appendStringInfo(&strInfo, "%s", sep);
				appendBinaryStringInfo(&strInfo, startPos, endPos - startPos);

				curr = endPos + 1;	/* skip \' */
			}
			else if(isdigit(*curr))	/* index of array field */
			{
				char *endPos = strstr(curr, prop_delim);

				appendStringInfoChar(&strInfo, '[');
				appendBinaryStringInfo(&strInfo, curr, endPos - curr);
				appendStringInfoChar(&strInfo, ']');
		...
		prevEnd = end + len_postfix;
	}

	/* print the end of the rest. */
	appendStringInfoString(&strInfo, prevEnd);

	return strInfo.data;
}

/* ----------
 * deparse_prop_expression_pretty
 *
						   ...
						   int prettyFlags, bool missing_ok)
{
	...
	if (!heap_attisnull(ht_idx, Anum_pg_index_indexprs))
	{
		...
		elog(ERROR, "Property index must be expression index.(%s)",
			 NameStr(idxrelrec->relname));
	...
	{
		/*
		 * Currently, must be EXCLUDE constraint and
		 * unique constraint uses EXCLUDE index
		 */
	...
	for (keyno = 0; keyno < idxrec->indnatts; keyno++)
	{
		...
		if (excludeOps != NULL)
		{
			char *op = generate_operator_name(excludeOps[keyno],
											  keycoltype,
											  keycoltype);

			if (strcmp(op, "=") != 0)
				elog(ERROR, "Invalid index property %u", idxrec->indexrelid);
		}
	}
	appendStringInfoChar(&buf, ')');
								 ...
								 bool missing_ok)
{
	...
	switch (conForm->contype)
	{
		...
		/* Unique constraint on agens-graph is implemented exclude index. */
		case CONSTRAINT_EXCLUSION:
			{
				Oid			indexOid = conForm->conindid;
				Datum		val;
		...
		case CONSTRAINT_UNIQUE:
		case CONSTRAINT_TRIGGER:
			elog(ERROR, "Invalid graph constraint type \"%c\"",
				 conForm->contype);
			break;
		default:
			elog(ERROR, "invalid constraint type \"%c\"", conForm->contype);

...
listGraphs(const char *pattern, bool verbose)
{
	...
	appendPQExpBuffer(&buf,
					  "\nFROM pg_catalog.pg_namespace n"
			  "\n	Right JOIN pg_catalog.ag_graph g ON g.nspid=n.oid\n");
...
{
	bool showEdges = labkind == 'v' ? false : true;
	bool showVertices = labkind == 'e' ? false : true;
	...
	appendPQExpBuffer(&buf,
					  "\nFROM pg_catalog.pg_class c"
			  "\n	Right JOIN pg_catalog.ag_label l ON c.oid=l.relid\n");

...
CREATE FUNCTION meta.get_last_graph_write_stats()
RETURNS TABLE(
    insertedvertices BIGINT,
    insertededges BIGINT,
    deletedvertices BIGINT,
    deletededges BIGINT,
    updatedproperties BIGINT
)
AS 'MODULE_PATHNAME', 'get_last_graph_write_stats'
LANGUAGE C STRICT;

COMMENT ON FUNCTION meta.get_last_graph_write_stats() IS
'Returns statistics of the last graph write operation (inserts, deletes, updates) in a session.';
